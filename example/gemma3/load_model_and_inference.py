...
def get_sample_for_forward(input_ids, token_type_ids, pad_token_id, hf_config, tp):
    ...
    ret = {}
    ret["input_ids"] = torch.LongTensor([input_ids]).to(
...
def main():
    ...
    pixel_values = torch.from_numpy(inputs.pixel_values[0]).unsqueeze(0).to(
        torch.cuda.current_device()
    ).to(torch.bfloat16)
    ...
    for _ in trange(max_new_tokens):
        sample = get_sample_for_forward(input_ids, token_type_ids, pad_token_id, bridge.hf_config,
                                        args.tp)
        with torch.no_grad():
