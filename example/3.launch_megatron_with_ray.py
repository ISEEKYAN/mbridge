#!/usr/bin/env python
"""
Example script to launch Megatron across multiple machines with Ray.

Prerequisites: Each machine has Ray, NCCL and CUDA correctly installed and configured.
...
Run this script from any machine (typically the head):
    python 3.launch_megatron_with_ray.py \
        --model_path /path/to/hf_model \
        --num_nodes 2 \
        --gpus_per_node 4 \
        --tp 2 --pp 1 --cp 1

The script starts world_size=num_nodes*gpus_per_node Ray workers, binds one GPU to each, sets all required distributed environment variables and launches Megatron.
"""
...
def init_distributed(
    ...
    """Initialize torch.distributed exactly as required by Megatron.
    ...
    # Ray controls the visible GPU list through CUDA_VISIBLE_DEVICES.
    # Inside the process those GPUs are renumbered to a contiguous range 0..N-1.
    # Because each Ray actor is assigned exactly one GPU we can safely use logical index 0.
...
def worker_fn(
    ...
    Loads the model and weights and optionally exports them. Shows how to correctly
    set up Megatron's distributed environment inside a Ray actor.
    """
...
def main():
    ...
    # Use the driver node IP as master_addr
