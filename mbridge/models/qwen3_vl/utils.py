...
def reorganize_inputs(
    ...
    if pixel_values is None:
        ...
        # Align video mask token count with video_grid_thw to eliminate
        # the discrepancy between HF tokenizer placeholder count and ViT output count.
        # Rollout responses may contain spurious video_token_id tokens generated by the
        # model; trim the mask to the grid_thw-implied count so IndexPutBackward0 sees
        # a consistent shape across forward and backward passes.
    ...
    if pixel_values_videos is None:
        ...
        # Align image mask token count with image_grid_thw to eliminate
        # the discrepancy between HF tokenizer placeholder count and ViT output count.
        # Rollout responses may contain spurious image_token_id tokens generated by the
        # model; trim the mask to the grid_thw-implied count so IndexPutBackward0 sees
        # a consistent shape across forward and backward passes.
        # Note: the mixed (image+video) path has an equivalent T*H*W assertion;
        # the pure-image early-return bypasses it, so we enforce the check here explicitly.
