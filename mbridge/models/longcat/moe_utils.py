...
class RouterGatingLinearFunction(torch.autograd.Function):
    ...
    def forward(ctx, inp: torch.Tensor, weight: torch.Tensor, router_dtype: torch.dtype):
        ...
        if te_general_gemm is not None and router_dtype != torch.float64 and weight.dtype != torch.float32:
    ...
    def backward(ctx, grad_output: torch.Tensor):
        ...
        if te_general_gemm is not None and ctx.router_dtype != torch.float64 and weight.dtype != torch.float32:
