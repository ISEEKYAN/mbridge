...
class ShortCutTransformerLayer(MegatronModule, BaseTransformerLayer):
    """A single shortcut transformer layer.

    ShortCutTransformer layer takes input with size [s, b, h] and returns an
    ...
    def __init__(
        ...
        for i in range(2):
            ...
            self_attn_bdas.append(
                build_module(submodules.self_attn_bda)
            )

            # [Module 4: Pre MLP] Layernorm post attn & before MLP
            ...
            # [Module 5: MLP]
            mlps.append(
                build_module(submodules.mlp, config=self.config, **additional_mlp_kwargs)
            )
        ...
        self.self_attention = nn.ModuleList(self_attn)
        self.self_attn_bda = self_attn_bdas  # Functions, not modules
        self.pre_mlp_layernorm = nn.ModuleList(post_attention_layernorm)
        self.mlp = nn.ModuleList(mlps)
        self.mlp_bda = mlp_bdas  # Functions, not modules
        # [Module 7: MoE]
        self.moe = build_module(submodules.moe, config=self.config, layer_number=self.layer_number, **additional_moe_kwargs)
    ...
    def forward(self, hidden_states, *args, **kwargs):
        ...
        for i in range(2):
            ...
            kwargs['attention_module_index'] = i
            hidden_states, context = self._forward_attention(hidden_states=hidden_states, *args, **kwargs)

            hidden_states = residual + hidden_states

            residual = hidden_states
            hidden_states = self.pre_mlp_layernorm[i](hidden_states)

            if i == 0:
                moe_output = self._forward_moe(hidden_states, kwargs.get("inference_context", None))

            hidden_states = self._forward_mlp(hidden_states, kwargs.get("inference_context", None), mlp_module_index=i)
    ...
    def _forward_attention(
        ...
        *,
        ...
        attention_module_index: int = None,
    ):
        """
        ...
        if self.recompute_input_layernorm:
            ...
            input_layernorm_output = self.input_layernorm_checkpoint.checkpoint(
                self.input_layernorm[attention_module_index], hidden_states
            )
        else:
            input_layernorm_output = self.input_layernorm[attention_module_index](hidden_states)

        # Self attention.
        attention_output_with_bias = self.self_attention[attention_module_index](
        ...
        return attention_output_with_bias[0], context

    def _forward_mlp(self, hidden_states, inference_context=None, mlp_module_index=None):
        """
        ...
        if self.recompute_mlp:
            mlp_output_with_bias = tensor_parallel.checkpoint(
                self.mlp[mlp_module_index], False, hidden_states
            )
        else:
            mlp_output_with_bias = self.mlp[mlp_module_index](hidden_states)
        ...
        output = make_viewless_tensor(
            inp=mlp_output_with_bias[0], requires_grad=hidden_states.requires_grad, keep_graph=True
        )

        return output

    def _forward_moe(self, hidden_states, inference_context=None):
        """
        Perform a forward pass through the feed-forward layer.
        ...
        if self.recompute_mlp:
            mlp_output_with_bias = tensor_parallel.checkpoint(
                self.moe, False, hidden_states
            )
        else:
            mlp_output_with_bias = self.moe(hidden_states)
    ...
    def sharded_state_dict(
        ...
        sharded_sd: ShardedStateDict = {}

        def merge(child_sd):
            for k, v in child_sd.items():
                sharded_sd[k] = v

        # ---- ModuleList: input_layernorm ----
        for i, ln in enumerate(self.input_layernorm):
            merge(
                sharded_state_dict_default(
                    ln,
                    prefix=f"{prefix}input_layernorm.{i}.",
                    sharded_offsets=sharded_offsets,
                    metadata=metadata,
                )
            )

        # ---- ModuleList: self_attention ----
        for i, attn in enumerate(self.self_attention):
            merge(
                sharded_state_dict_default(
                    attn,
                    prefix=f"{prefix}self_attention.{i}.",
                    sharded_offsets=sharded_offsets,
                    metadata=metadata,
                )
            )

        # ---- ModuleList: pre_mlp_layernorm ----
        for i, ln in enumerate(self.pre_mlp_layernorm):
            merge(
                sharded_state_dict_default(
                    ln,
                    prefix=f"{prefix}pre_mlp_layernorm.{i}.",
                    sharded_offsets=sharded_offsets,
                    metadata=metadata,
                )
            )
        ...
        if prefixed_map:
            apply_prefix_mapping(sharded_sd, prefixed_map)
