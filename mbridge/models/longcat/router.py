...
class Router(ABC, MegatronModule):
    ...
    def __init__(
        ...
        self.weight = torch.nn.Parameter(
            torch.empty((self.config.num_moe_experts + self.config.zero_expert_num, self.config.hidden_size), dtype=torch.float32)
        )
...
class TopKRouter(Router):
    ...
    def __init__(
        ...
        if self.enable_expert_bias:
            self.register_buffer(
                ...
                torch.zeros(self.config.num_moe_experts + self.config.zero_expert_num, dtype=torch.float32),
                persistent=False,
            )
            self.register_buffer(
                'expert_bias', torch.zeros(self.config.num_moe_experts + self.config.zero_expert_num, dtype=torch.float32)
            )
        else:
    ...
    def routing(self, logits: torch.Tensor):
        ...
        logits = logits.view(-1, self.config.num_moe_experts + self.config.zero_expert_num)
