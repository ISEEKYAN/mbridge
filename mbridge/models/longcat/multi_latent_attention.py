...
class MLASelfAttention(MultiLatentAttention):
    ...
    def __init__(
        ...
        self.apply_qkv_down_proj_fusion = (
            self.config.q_lora_rank is not None
            and submodules.linear_q_down_proj == submodules.linear_kv_down_proj
        )
        self._init_linear_qkv_down(submodules)
        ...
        self.mla_scale_q_lora = (config.hidden_size / config.q_lora_rank) ** 0.5
        self.mla_scale_kv_lora = (config.hidden_size / config.kv_lora_rank) ** 0.5

    def _init_linear_qkv_down(self, submodules: MLASelfAttentionSubmodules):
        def init_down_proj_kwargs(proj_module: Union[ModuleSpec, type], module_name: str):
            down_proj_kwargs = {}
            if proj_module in [TELinear]:
                down_proj_kwargs['parallel_mode'] = 'duplicated'
            elif proj_module in [Linear, TEColumnParallelLinear, ColumnParallelLinear]:
                down_proj_kwargs['gather_output'] = False
            else:
                raise ValueError(f"Unsupported {module_name}: {proj_module}")

            return down_proj_kwargs

        if self.apply_qkv_down_proj_fusion:
            self.linear_qkv_down_proj = build_module(
                ...
                **init_down_proj_kwargs(submodules.linear_q_down_proj, "linear_qkv_down_proj"),
            )
        else:
            if self.config.q_lora_rank is not None:
                self.linear_q_down_proj = build_module(
                    ...
                    skip_weight_param_allocation=False,
                    **init_down_proj_kwargs(submodules.linear_q_down_proj, "linear_q_down_proj"),
                )
            ...
            self.linear_kv_down_proj = build_module(
                ...
                skip_weight_param_allocation=False,
                **init_down_proj_kwargs(submodules.linear_kv_down_proj, "linear_kv_down_proj"),
            )
    ...
    def get_query_key_value_tensors(
        ...
        *,
        ...
        if self.config.apply_rope_fusion:
            ...
            if self.config.rope_type == "rope":
                emb = self.rotary_pos_emb(rotary_seq_len, packed_seq=packed_seq)
                rotary_pos_cos, rotary_pos_sin = emb.cos().contiguous(), emb.sin().contiguous()
            else:
        ...
        # =========================================
        def unfused_qkv_down_proj(hidden_states):
            if self.config.q_lora_rank is not None:
            ...
            return q_compressed, kv_compressed, k_pos_emb

        def fused_qkv_down_proj(hidden_states):
            # if linear_qkv_down_proj is ColumnParallelLinear:
            #     qkv_compressed: [s, b, (q_lora_rank + kv_lora_rank + qk_pos_emb_head_dim) / TP]
            # elif linear_qkv_down_proj is Linear:
            #     qkv_compressed: [s / TP, b, (q_lora_rank + kv_lora_rank + qk_pos_emb_head_dim)]
            qkv_compressed, _ = self.linear_qkv_down_proj(hidden_states)

            # When output is sharded (ColumnParallelLinear), two things are needed to be
            # identical to a normal Linear.
            #   1. Manually gather output to restore output dim
            #      (q_lora_rank + kv_lora_rank + qk_pos_emb_head_dim);
            #   2. Scatter sequence back to s / TP if sequence-parallel since it was
            #      gathered by ColumnParallelLinear.
            if qkv_compressed.size(-1) != (
                self.config.q_lora_rank + self.config.kv_lora_rank + self.config.qk_pos_emb_head_dim
            ):
                qkv_compressed = gather_from_tensor_model_parallel_region(qkv_compressed)
        ...
        q_compressed, kv_compressed, k_pos_emb = (
            fused_qkv_down_proj(hidden_states)
            if self.apply_qkv_down_proj_fusion
            else unfused_qkv_down_proj(hidden_states)
        )
        ...
        def qkv_up_proj_and_rope_apply(q_compressed, kv_compressed, k_pos_emb, rotary_pos_emb):
            ...
            kv_compressed = self.kv_layernorm(kv_compressed)
            kv_compressed = kv_compressed * self.mla_scale_kv_lora
