...
def get_adapter_attributes_from_linear(m: nn.Module, is_expert: bool = False) -> AdapterAttributes:
    ...
    if isinstance(m, TopKRouter):
        ...
        disable_sequence_parallel_comm = True
    elif any(isinstance(m, te_column_parallel) for te_column_parallel in TECL):
        input_is_parallel = False
        ...
        if isinstance(m, TELayerNormColumnParallelLinear):
            ...
            if hasattr(m, "config") and m.config.sequence_parallel and not ub_overlap_ag:
                ...
                if te_version >= packaging.version.Version("1.5.0dev") and (
                    ...
                    disable_sequence_parallel_comm = True
    elif any(isinstance(m, te_row_parallel) for te_row_parallel in TERL):
        input_is_parallel = True
        in_features = m.in_features * tp_size
        out_features = m.out_features
    elif isinstance(m, TELinear):  # parallel_mode="duplicated"
        input_is_parallel = False
        in_features = m.in_features
        out_features = m.out_features
        base_linear_is_parallel = False
...
class ParallelLinearAdapter(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.dim = dim
        self.in_features = in_features  # stored for TP-independent init
        ...
        # Force CPU initialization so lora_A (xavier) is TP-independent:
        # GPU init uses per-rank model-parallel seeds (seed + tp_rank), producing
        # different full weights for different TP degrees.  CPU init generates
        # one master weight on all ranks with the same RNG and scatters the
        # matching shard, making the result identical regardless of TP.
        _use_cpu_init = getattr(model_parallel_config, 'use_cpu_initialization', False)
        model_parallel_config.use_cpu_initialization = True
        ...
        # revert config changes in case they are read elsewhere
        model_parallel_config.use_cpu_initialization = _use_cpu_init
    ...
    def _get_init_fn(self, init_method: str) -> Callable[[torch.Tensor], torch.Tensor]:
        ...
        Note:
            For ``xavier`` init on ``linear_in`` (lora_A), we use the full (unsharded)
            ``dim`` and ``in_features`` to compute the xavier std, making initialization
            independent of tensor_model_parallel_size. Otherwise, with TP>1 the per-rank
            weight shape is ``(dim/TP, in_features)`` and xavier computes a different std
            (because ``fan_out = dim/TP``), leading to diverging training trajectories.
        """
        if init_method == "xavier":
            # Use the full (unsharded) dim to make init TP-independent.
            # xavier_normal_ uses std = gain * sqrt(2 / (fan_in + fan_out)).
            # fan_in = self.in_features, fan_out = self.dim (full, not dim/TP).
            std = math.sqrt(2.0 / (self.in_features + self.dim))
            init_fn = init_method_normal(std)
